...
def run_pmu_sync():
    ...
    # Only keep course name updates for courses that already exist
    course_updates = {}
    ...
    for section in sections_data:
        ...
        if course_id in existing_courses:
            ...
            course_updates[course_id] = {
                "id": course_id,
                "name": course_title,
                "credits": existing_courses[course_id]["credits"]
            }
        else:
    ...
    courses_data = list(course_updates.values())

    # Update existing courses only
    course_response = None
    if courses_data:
        course_response = supabase.table("courses").upsert(courses_data).execute()
    ...
    section_response = supabase.table("course_sections").upsert(enriched_sections).execute()
    meeting_response = supabase.table("course_section_meetings").upsert(filtered_meetings).execute()
    ...
    return {
        ...
        "scraped_course_count": len(course_updates),
